...
define :create_fx_channel_play do |channel, instru, note, fx_index, fxs_name|
  if fxs_name.length == 0 or fx_index >= fxs_name.length then
    set "play_on_#{channel}_#{note}".to_sym, synth instru[:name].to_sym, instru[:options].to_h.merge({:note => note}) if instru[:type] == 'synth'
    set "play_on_#{channel}".to_sym, sample instru[:name], instru[:options].to_h if instru[:type] == 'sample' or instru[:type] == 'external_sample'
  else
